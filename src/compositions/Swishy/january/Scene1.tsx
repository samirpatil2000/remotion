...
function Scene() {
  ...
  const weekdays = SCENE_PARAMS.weekdays.value.split("\n").slice(0, 7);
  const totalDays = Math.max(28, Math.min(31, Math.round(SCENE_PARAMS.daysInMonth.value)));
  const startIndex = Math.max(0, Math.min(6, Math.round(SCENE_PARAMS.startDayIndex.value)));
  ...
  const weekdaysY = interpolate(weekdaysProgress, [0, 1], [SCENE_PARAMS.entranceOffset.value, 0], { extrapolateRight: "clamp" });
      ...
      <div style={{
        ...
        <div style={{ position: "absolute", left: gridLeft, top: isPortrait ? height * 0.1 : height * 0.08 }}>
          <div style={{
            ...
            opacity: interpolate(titleProgress, [0, 1], [0, SCENE_PARAMS.opacity.value], { extrapolateRight: "clamp" }),
            transform: "translateY(" + titleY + "px)",
            lineHeight: 1.05
          }}>{SCENE_PARAMS.month.value}</div>
          <div style={{
        ...
        <div style={{
          ...
          gap: cellGap,
          opacity: interpolate(weekdaysProgress, [0, 1], [0, SCENE_PARAMS.opacity.value], { extrapolateRight: "clamp" }),
        ...
        <div style={{
          ...
          {daysArray.map((day, i) => {
            const delay = daysStartDelay + i * SCENE_PARAMS.staggerDelay.value;
            ...
            const isHighlight = day === Math.round(SCENE_PARAMS.highlightDay.value);
            const popProgress = spring({
              frame: Math.max(0, adjustedFrame - delay - SCENE_PARAMS.highlightDelay.value),
              fps,
              config: { damping: 18, stiffness: 120 }
            });
            const popScale = isHighlight
              ? interpolate(popProgress, [0, 1], [1, SCENE_PARAMS.highlightScale.value], { extrapolateRight: "clamp" })
              : 1;
            return (
              <div key={day} style={{
                ...
                fontFamily: SCENE_PARAMS.bodyFont.value + ", system-ui, sans-serif",
                textAlign: "center",
                opacity: interpolate(progress, [0, 1], [0, SCENE_PARAMS.opacity.value], { extrapolateRight: "clamp" }),
